...
from ..common.compat import scandir, text_type, on_win, encode_environment
    ...
    def get_hook_executables():
        try:
            paths = map(lambda entry: entry.path,
                        scandir(join(context.root_prefix, 'etc', 'conda', 'create.d')))
        except EnvironmentError:
            return ()
        return sorted(filter(lambda p: os.access(p, os.X_OK), paths))

    for executable in get_hook_executables():
        script_caller, command = wrap_subprocess_call(on_win, context.root_prefix, prefix,
                                                      dev_mode=False, debug_wrapper_scripts=False,
        ...
        if response.rc != 0:
            log.error("Subprocess for '{}' command failed.".format(executable))
